...
df_status df_write_json_report(const char *path,
                               const df_report_context *context,
                               df_error *error) {
    FILE *file;
    ...
    if (path == NULL || context == NULL) {
        df_error_set(error, DF_ERR_INVALID_ARGUMENT, 0,
                     "report path and context are required");
        return DF_ERR_INVALID_ARGUMENT;
    }
    file = fopen(path, "wb");
    if (file == NULL) {
        df_error_set(error, DF_ERR_OPEN, 0,
                     "could not create report '%s'", path);
        return DF_ERR_OPEN;
    }

    if (context->write_result != NULL) {
        df_hex_encode(context->write_result->source_sha256,
                      32u, source_hash);
        df_hex_encode(context->write_result->target_sha256,
                      32u, target_hash);
    }
    ...
    json_string(file, context->operation);
    fputs(",\n", file);
    fputs("  \"status\": ", file);
    json_string(file, df_status_name(context->status));
    fputs(",\n", file);
    fputs("  \"source\": ", file);
    json_string(file, context->source_path);
    fputs(",\n", file);
    fputs("  \"target_path\": ", file);
    json_string(file, context->target_path);
    fputs(",\n", file);
    fputs("  \"plan_seal\": ", file);
    if (context->plan_seal != NULL) {
        json_string(file, context->plan_seal);
    } else {
        fputs("null", file);
    }
    fputs(",\n", file);
    ...
    if (context->target != NULL) {
        ...
        fprintf(file, "    \"read_only\": %s,\n",
                context->target->read_only ? "true" : "false");
        ...
        json_string(file,
                    context->target->serial_bound ? "serial_bound" :
                    (context->target->descriptor_present ?
                         "descriptor_bound" : "geometry_only"));
        fputs(",\n", file);
        fputs("    \"bus_type\": ", file);
        json_string(file, context->target->bus_type);
        fputs(",\n", file);
        fputs("    \"vendor\": ", file);
        json_string(file, context->target->vendor);
        fputs(",\n", file);
        fputs("    \"product\": ", file);
        json_string(file, context->target->product);
        fputs(",\n", file);
        fputs("    \"revision\": ", file);
        json_string(file, context->target->revision);
        fputs(",\n", file);
        fputs("    \"serial_sha256\": ", file);
        json_string(file, context->target->serial_sha256);
        fputs(",\n", file);
        fputs("    \"token\": ", file);
        json_string(file, context->target->token);
        fputs("\n  },\n", file);
    } else {
        fputs("  \"target\": null,\n", file);
    }
    ...
    if (context->write_options != NULL) {
        ...
        fputs("    \"verify_mode\": ", file);
        json_string(file,
                    verify_mode_name(context->write_options->verify_mode));
        fputs(",\n", file);
        fprintf(file, "    \"sample_count\": %u,\n",
                context->write_options->sample_count);
        fprintf(file, "    \"allow_device\": %s,\n",
                context->write_options->allow_device ? "true" : "false");
        fprintf(file, "    \"force_mounted\": %s,\n",
                context->write_options->force_mounted ? "true" : "false");
        fprintf(file, "    \"force_system_disk\": %s,\n",
                context->write_options->force_system_disk ?
                    "true" : "false");
        fprintf(file, "    \"truncate_regular_file\": %s,\n",
                context->write_options->truncate_regular_file ?
                    "true" : "false");
    ...
    if (context->write_result != NULL) {
        ...
        fputs("  \"result\": {\n", file);
        fputs("    \"state\": ", file);
        json_string(file, r->final_state);
        fputs(",\n", file);
        ...
        fprintf(file, "    \"write_retries\": %" PRIu64 ",\n",
                r->write_retries);
        fprintf(file,
                "    \"verification_mismatches\": %" PRIu64 ",\n",
        ...
        fprintf(file, "    \"write_mib_s\": %.3f,\n",
                r->write_ms > 0.0 ?
                    ((double)r->bytes_written / 1048576.0) /
                        (r->write_ms / 1000.0) : 0.0);
        fputs("    \"source_sha256\": ", file);
        json_string(file, source_hash);
        fputs(",\n", file);
        fputs("    \"target_sha256\": ", file);
        json_string(file, target_hash);
        fputs("\n  },\n", file);
    } else {
        fputs("  \"result\": null,\n", file);
    }
    ...
    if (context->fat32_layout != NULL) {
        const df_fat32_layout *layout = context->fat32_layout;
        fputs("  \"fat32\": {\n", file);
    ...
    json_string(file,
                context->error != NULL ?
                    df_status_name(context->error->code) : "ok");
    fputs(", \"os_code\": ", file);
    fprintf(file, "%d",
            context->error != NULL ? context->error->os_code : 0);
    fputs(", \"message\": ", file);
    json_string(file,
                context->error != NULL ? context->error->message : "");
    fputs("}\n}\n", file);

    if (fclose(file) != 0) {
        df_error_set(error, DF_ERR_IO, 0,
                     "could not flush report '%s'", path);
        return DF_ERR_IO;
    }
    return DF_OK;
}
