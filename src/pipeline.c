...
static size_t df_round_up_size(size_t value, size_t alignment) {
    ...
    if (value > SIZE_MAX - (alignment - remainder)) return 0u;
...
static uint64_t df_round_up_u64(uint64_t value, uint64_t alignment) {
    ...
    remainder = value % alignment;
    if (remainder == 0u) return value;
    if (value > UINT64_MAX - (alignment - remainder)) return 0u;
    return value + alignment - remainder;
}
...
const char *df_progress_phase_name(df_progress_phase phase) {
    switch (phase) {
        case DF_PROGRESS_PREPARE: return "prepare";
        case DF_PROGRESS_HASH_SOURCE: return "hash_source";
        case DF_PROGRESS_WRITE: return "write";
        case DF_PROGRESS_FLUSH: return "flush";
        case DF_PROGRESS_VERIFY: return "verify";
        case DF_PROGRESS_COMPLETE: return "complete";
        default: return "unknown";
    }
}

static void df_notify(df_progress_callback callback, void *context,
                      df_progress_phase phase, uint64_t completed, uint64_t total) {
    if (callback != NULL) callback(context, phase, completed, total);
}

static df_status df_hash_file_region_progress(df_file *file, uint64_t length,
                                               size_t buffer_size,
                                               uint8_t digest[32],
                                               df_progress_callback callback,
                                               void *context,
    ...
    df_notify(callback, context, phase, 0u, length);
    df_sha256_init(&hash);
    while (offset < length) {
        size_t wanted = (size_t)((length - offset) < buffer_size ?
                                 (length - offset) : buffer_size);
        ...
        if (file->direct_io) {
            ...
            if (request == 0u || request > allocation_size) {
                df_aligned_free(buffer);
                df_error_set(error, DF_ERR_INTERNAL, 0,
                             "invalid direct-I/O hash request");
        ...
        if (got < wanted) {
            ...
            df_error_set(error, DF_ERR_IO, 0,
                         "unexpected end of file at offset %" PRIu64, offset);
            return DF_ERR_IO;
        }

        df_sha256_update(&hash, buffer, wanted);
        offset += wanted;
        df_notify(callback, context, phase, offset, length);
    }
...
df_status df_hash_file_region(df_file *file, uint64_t length, size_t buffer_size,
    ...
    return df_hash_file_region_progress(file, length, buffer_size, digest,
                                        NULL, NULL, DF_PROGRESS_HASH_SOURCE,
                                        error);
}
...
df_status df_hash_source_path(const char *path, size_t buffer_size,
    ...
    if (path == NULL || size_bytes == NULL || digest == NULL) {
        df_error_set(error, DF_ERR_INVALID_ARGUMENT, 0,
                     "source hash requires path, size, and digest outputs");
        return DF_ERR_INVALID_ARGUMENT;
    }
    if (buffer_size == 0u) buffer_size = DF_DEFAULT_BUFFER_SIZE;

    df_pipeline_file_reset(&source);
    status = df_open_source(path, &source, error);
    if (status != DF_OK) return status;

    *size_bytes = source.size_bytes;
    status = df_hash_file_region_progress(&source, source.size_bytes,
                                          buffer_size, digest,
                                          callback, context,
                                          DF_PROGRESS_HASH_SOURCE, error);
...
static uint64_t df_seed_from_hash(const uint8_t digest[32]) {
    uint64_t seed = 0u;
    size_t i;
    for (i = 0u; i < 8u; ++i) seed = (seed << 8) | digest[i];
    return seed != 0u ? seed : UINT64_C(0xdeadf1a5c0ffee);
}
...
static df_status df_verify_sample_files(df_file *source, df_file *target,
    ...
    if (sample_count == 0u) sample_count = DF_DEFAULT_SAMPLE_COUNT;
    block_size = source->size_bytes < DF_SAMPLE_BLOCK_SIZE ?
                 (size_t)source->size_bytes : DF_SAMPLE_BLOCK_SIZE;
    if (block_size == 0u) {
        df_notify(callback, context, DF_PROGRESS_VERIFY, 0u, 0u);
        return DF_OK;
    }

    iterations = (uint64_t)sample_count + 2u;
    progress_total = iterations * (uint64_t)block_size;
    left = (uint8_t *)df_aligned_alloc(4096u,
                                       df_round_up_size(block_size, 4096u));
    right = (uint8_t *)df_aligned_alloc(4096u,
                                        df_round_up_size(block_size, 4096u));
    if (left == NULL || right == NULL) {
        df_aligned_free(left);
        df_aligned_free(right);
        df_error_set(error, DF_ERR_NO_MEMORY, 0,
                     "could not allocate sample verification buffers");
        return DF_ERR_NO_MEMORY;
    }

    seed = df_seed_from_hash(source_hash);
    df_notify(callback, context, DF_PROGRESS_VERIFY, 0u, progress_total);
    for (iteration = 0u; iteration < iterations; ++iteration) {
        ...
        if (iteration == 0u) offset = 0u;
        else if (iteration == 1u) offset = max_offset;
        else offset = max_offset == 0u ? 0u :
                      df_prng_next(&seed) % (max_offset + 1u);

        status = df_read_at(source, left, block_size, offset,
                            &got_left, error);
        if (status != DF_OK) goto fail;
        status = df_read_at(target, right, block_size, offset,
                            &got_right, error);
        if (status != DF_OK) goto fail;
        ...
        if (got_left != block_size || got_right != block_size ||
            ...
            df_error_set(error, DF_ERR_VERIFY_MISMATCH, 0,
                         "sample verification mismatch at offset %" PRIu64,
                         offset);
        ...
        df_notify(callback, context, DF_PROGRESS_VERIFY,
                  result->bytes_verified, progress_total);
    }
...
static df_status df_verify_image_internal(const char *source_path,
                                           const char *target_path,
                                           df_verify_mode mode,
                                           unsigned sample_count,
                                           size_t buffer_size,
                                           const uint8_t *known_source_hash,
                                           df_progress_callback callback,
                                           void *context,
                                           df_write_result *result,
                                           df_error *error) {
    ...
    if (source_path == NULL || target_path == NULL || result == NULL) {
        df_error_set(error, DF_ERR_INVALID_ARGUMENT, 0,
                     "verify requires source, target, and result");
        return DF_ERR_INVALID_ARGUMENT;
    }
    ...
    if (target.size_bytes != 0u && target.size_bytes < source.size_bytes) {
    ...
    if (known_source_hash != NULL) {
        memcpy(source_hash, known_source_hash, sizeof(source_hash));
    } else {
        status = df_hash_file_region_progress(&source, source.size_bytes,
                                              buffer_size, source_hash,
                                              callback, context,
                                              DF_PROGRESS_HASH_SOURCE, error);
        if (status != DF_OK) goto out;
    }
    memcpy(result->source_sha256, source_hash, sizeof(source_hash));
    ...
    if (mode == DF_VERIFY_NONE) {
        ...
        status = df_verify_sample_files(&source, &target, source_hash,
                                        sample_count, result,
                                        callback, context, error);
    } else {
        status = df_hash_file_region_progress(&target, source.size_bytes,
                                              buffer_size, target_hash,
                                              callback, context,
                                              DF_PROGRESS_VERIFY, error);
        if (status == DF_OK) {
            memcpy(result->target_sha256, target_hash, sizeof(target_hash));
            result->bytes_verified = source.size_bytes;
            if (!df_constant_time_equal(source_hash, target_hash,
                                        sizeof(source_hash))) {
                result->verification_mismatches = 1u;
                df_error_set(error, DF_ERR_VERIFY_MISMATCH, 0,
                             "full verification hash mismatch");
                status = DF_ERR_VERIFY_MISMATCH;
            }
        }
    }
    ...
    (void)snprintf(result->final_state, sizeof(result->final_state), "%s",
                   status == DF_OK ?
                       (mode == DF_VERIFY_NONE ?
                           "success_unverified" : "success_verified") :
                       "verification_failed");
...
df_status df_verify_image(const char *source_path, const char *target_path,
                          df_verify_mode mode, unsigned sample_count,
                          size_t buffer_size, df_write_result *result,
                          df_error *error) {
    return df_verify_image_internal(source_path, target_path, mode,
                                    sample_count, buffer_size, NULL,
...
df_status df_write_image(const char *source_path, const char *target_path,
    ...
    if (source_path == NULL || target_path == NULL || result == NULL) {
        df_error_set(error, DF_ERR_INVALID_ARGUMENT, 0,
                     "write requires source, target, and result");
        return DF_ERR_INVALID_ARGUMENT;
    }
    ...
    retries_default = options.write_retries == 0u ?
                      DF_DEFAULT_WRITE_RETRIES : options.write_retries;
    options.write_retries = retries_default;
    if (options.sample_count == 0u) {
        options.sample_count = DF_DEFAULT_SAMPLE_COUNT;
    }
    ...
    df_notify(options.progress_callback, options.progress_context,
              DF_PROGRESS_PREPARE, 0u, 1u);
    ...
    if (planned.kind != DF_TARGET_REGULAR_FILE &&
        df_round_up_u64(source.size_bytes,
                        planned.logical_sector_size) > planned.size_bytes) {
    ...
    if (buffer == NULL) {
        df_error_set(error, DF_ERR_NO_MEMORY, 0,
                     "could not allocate write buffer");
        status = DF_ERR_NO_MEMORY;
        goto out;
    }

    df_notify(options.progress_callback, options.progress_context,
              DF_PROGRESS_WRITE, 0u, source.size_bytes);
    ...
    while (offset < source.size_bytes) {
        ...
        if (got != wanted) {
            df_error_set(error, DF_ERR_IO, 0,
                         "short source read at offset %" PRIu64, offset);
            status = DF_ERR_IO;
            goto out;
        }

        df_sha256_update(&write_hash, buffer, wanted);
        if (target.is_device && wanted % target.alignment != 0u) {
            write_length = df_round_up_size(wanted, target.alignment);
            if (write_length == 0u || write_length > buffer_size) {
                df_error_set(error, DF_ERR_INTERNAL, 0,
                             "invalid final aligned write size");
        ...
        for (attempt = 0u; attempt < options.write_retries; ++attempt) {
            size_t written = 0u;
            status = df_write_at(&target, buffer, write_length, offset,
                                 &written, error);
            if (status == DF_OK && written == write_length) break;
            result->write_retries++;
            if (status == DF_OK) {
                df_error_set(error, DF_ERR_IO, 0,
                             "short target write at offset %" PRIu64
                             ": %zu/%zu",
        ...
        df_notify(options.progress_callback, options.progress_context,
                  DF_PROGRESS_WRITE, result->bytes_written,
                  source.size_bytes);
    }
    ...
    if (options.verify_mode != DF_VERIFY_NONE) {
        ...
        result->verification_mismatches =
            verify_result.verification_mismatches;
        memcpy(result->target_sha256, verify_result.target_sha256,
               sizeof(result->target_sha256));
        if (status != DF_OK) {
